...
@FeignClient(name = "ares-cloud-audit", contextId = "authOracleHsmClient", configuration = FeignConfig.class, fallback = AuthOracleHsmClientFallback.class)
public interface AuthOracleHsmClient {

    /**
     * 生成签名
     */
    @PostMapping("/api/audit/oracle/hsm/v1/sign")
    String sign(@Validated(OcidHsmDto.SignGroup.class) @RequestBody OcidHsmDto ocidHsmDto);

    /**
     * 验签
     */
    @PostMapping("/api/audit/oracle/hsm/v1/verify")
    Boolean verify(@Validated(OcidHsmDto.Verify.class) @RequestBody OcidHsmDto ocidHsmDto);

    /**
     * 加密
     */
    @PostMapping("/api/audit/oracle/hsm/v1/encrypt")
    String encrypt(@Validated(OcidHsmDto.Encrypt.class) @RequestBody OcidHsmDto ocidHsmDto);

    /**
     * 解密
     */
    @PostMapping("/api/audit/oracle/hsm/v1/decrypt")
